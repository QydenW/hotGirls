...
function ensureInfo(tech) {
  ...
  tech.info.scores ||= {};
  for (const key of ["age", "singing"]) tech.info.scores[key] ||= "";
  tech.media ||= [];
}
...
function addTech() {
  ...
  techs.push({
    ...
    avatar: "",
    media: [],
    info: { number: base, scores: { age: "", singing: "" }, comment: "" }
  });
  selectedNumber = base;
  setStatus("已新增技师，填写资料后点击保存信息");
  renderAll();
}
...
[els.number, els.category, els.age, els.singing, els.comment].forEach((el) => {
...
els.saveInfo.addEventListener("click", saveAll);
